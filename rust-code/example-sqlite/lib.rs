...
impl DbExecutable {
    /// Only use if you have no injected values to specify
...
impl<T: FromQuery> DbSelectable<T> {
    pub fn new(statement: String, values: Vec<SqlValue>, columns: Vec<ColumnType>) -> Self {
        DbSelectable {
            executable: DbExecutable { statement, values },
            columns,
            _phantom: std::marker::PhantomData,
        }
    }
    pub fn parse_values(&self, values: Vec<SqlValue>) -> Result<T, SwormError> {
        T::parse(values)
    }
}
...
impl SqliteDb {
    fn open(filename: &str) -> Result<Self, SwormError> {
    ...
    fn execute<T: FromQuery>(&self, selectable: DbSelectable<T>) -> Result<Vec<T>, SwormError> {
        if cfg!(debug_assertions) {
    ...
    fn execute_only(&self, executable: DbExecutable) -> Result<(), SwormError> {
        if cfg!(debug_assertions) {
...
pub trait FromQuery: Sized {
    fn parse(values: Vec<SqlValue>) -> Result<Self, SwormError>;
}
fn get_rows<T: FromQuery>(
    stmt_ptr: *mut sqlite3_stmt,
    executable: &DbSelectable<T>,
) -> Result<Vec<T>, SwormError> {
                ...
                match column_type {
                    ...
                    ColumnType::Date => {
                        ...
                        let date = UNIX_EPOCH
                            ...
                            .ok_or_else(|| SwormError::SystemTimeError())?;
    ...
    if step_code == SQLITE_DONE {
        ...
        Err(SwormError::StepFailed(error_message, step_code))
    }
}
#[derive(Debug)]
pub enum SqlValue {
...
mod tests {
    use crate::{
        ColumnType, DbExecutable, DbSelectable, FromQuery, SqlValue, SqliteDb, SwormError,
    };
    ...
    impl FromQuery for Person {
        fn parse(values: Vec<SqlValue>) -> Result<Self, SwormError> {
            if values.len() != 3 {
                return Err(SwormError::ParseInvalidLength(values.len(), 3));
            }

            let mut iter = values.into_iter();
            let id = match iter.next().unwrap() {
                SqlValue::Int(i) => Ok(i),
                v => Err(SwormError::ParseInvalidType {
            ...
            let data = match iter.next().unwrap() {
                SqlValue::Blob(i) => Ok(Some(i)),
